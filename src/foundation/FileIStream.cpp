...
{
    LuaPoco::FileIStreamUserdata::registerFileIStream(L);
...
int FileIStreamUserdata::FileIStream(lua_State* L)
{
    ...
    FileIStreamUserdata* fisud = NULL;
    void* p = lua_newuserdata(L, sizeof *fisud);
    
    try
    {
        fisud = new(p) FileIStreamUserdata(path);
    }
